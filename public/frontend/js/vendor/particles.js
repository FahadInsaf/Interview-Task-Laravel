...
var pJS = function(e, t) {
    ...
    t && Object.deepExtend(p, t), p.tmp.obj = {
        ...
        this.radius = (p.particles.size.random ? Math.random() : 1) * p.particles.size.value, p.particles.size.anim.enable && (this.size_status = !1, this.vs = p.particles.size.anim.speed / 100, p.particles.size.anim.sync || (this.vs = this.vs * Math.random())), this.x = a ? a.x : Math.random() * p.canvas.w, this.y = a ? a.y : Math.random() * p.canvas.h, this.x > p.canvas.w - 2 * this.radius ? this.x = this.x - this.radius : this.x < 2 * this.radius && (this.x = this.x + this.radius), this.y > p.canvas.h - 2 * this.radius ? this.y = this.y - this.radius : this.y < 2 * this.radius && (this.y = this.y + this.radius), p.particles.move.bounce && p.fn.vendors.checkOverlap(this, a), this.color = {}, "object" == typeof e.value ? e.value instanceof Array ? (n = e.value[Math.floor(Math.random() * p.particles.color.value.length)], this.color.rgb = hexToRgb(n)) : (null != e.value.r && null != e.value.g && null != e.value.b && (this.color.rgb = {
            r: e.value.r,
            g: e.value.g,
            b: e.value.b
        ...
        "object" == typeof n ? n instanceof Array && (e = n[Math.floor(Math.random() * n.length)], this.shape = e) : this.shape = n, "image" == this.shape && (n = p.particles.shape, this.img = {
            src: n.image.src,
        ...
        for (var e = 0; e < p.particles.array.length; e++) {
            ...
            if (p.particles.move.enable && (t = p.particles.move.speed / 2, a.x += a.vx * t, a.y += a.vy * t), p.particles.opacity.anim.enable && (1 == a.opacity_status ? (a.opacity >= p.particles.opacity.value && (a.opacity_status = !1), a.opacity += a.vo) : (a.opacity <= p.particles.opacity.anim.opacity_min && (a.opacity_status = !0), a.opacity -= a.vo), a.opacity < 0 && (a.opacity = 0)), p.particles.size.anim.enable && (1 == a.size_status ? (a.radius >= p.particles.size.value && (a.size_status = !1), a.radius += a.vs) : (a.radius <= p.particles.size.anim.size_min && (a.size_status = !0), a.radius -= a.vs), a.radius < 0 && (a.radius = 0)), t = "bounce" == p.particles.move.out_mode ? {
                ...
                }, "bounce" === (a.x - a.radius > p.canvas.w ? (a.x = t.x_left, a.y = Math.random() * p.canvas.h) : a.x + a.radius < 0 && (a.x = t.x_right, a.y = Math.random() * p.canvas.h), a.y - a.radius > p.canvas.h ? (a.y = t.y_top, a.x = Math.random() * p.canvas.w) : a.y + a.radius < 0 && (a.y = t.y_bottom, a.x = Math.random() * p.canvas.w), p.particles.move.out_mode) && ((a.x + a.radius > p.canvas.w || a.x - a.radius < 0) && (a.vx = -a.vx), (a.y + a.radius > p.canvas.h || a.y - a.radius < 0) && (a.vy = -a.vy)), isInArray("grab", p.interactivity.events.onhover.mode) && p.fn.modes.grabParticle(a), (isInArray("bubble", p.interactivity.events.onhover.mode) || isInArray("bubble", p.interactivity.events.onclick.mode)) && p.fn.modes.bubbleParticle(a), (isInArray("repulse", p.interactivity.events.onhover.mode) || isInArray("repulse", p.interactivity.events.onclick.mode)) && p.fn.modes.repulseParticle(a), p.particles.line_linked.enable || p.particles.move.attract.enable)
        ...
        var e, t, i, n, s, r, c;
        p.interactivity.events.onhover.enable && isInArray("repulse", p.interactivity.events.onhover.mode) && "mousemove" == p.interactivity.status ? (t = a.x - p.interactivity.mouse.pos_x, i = a.y - p.interactivity.mouse.pos_y, e = t / (r = Math.sqrt(t * t + i * i)), t = i / r, r = clamp(1 / (i = p.interactivity.modes.repulse.distance) * (-1 * Math.pow(r / i, 2) + 1) * i * 100, 0, 50), r = {
            x: a.x + e * r,
            y: a.y + t * r
        }, "bounce" == p.particles.move.out_mode ? (0 < r.x - a.radius && r.x + a.radius < p.canvas.w && (a.x = r.x), 0 < r.y - a.radius && r.y + a.radius < p.canvas.h && (a.y = r.y)) : (a.x = r.x, a.y = r.y)) : p.interactivity.events.onclick.enable && isInArray("repulse", p.interactivity.events.onclick.mode) && (p.tmp.repulse_finish || (p.tmp.repulse_count++, p.tmp.repulse_count == p.particles.array.length && (p.tmp.repulse_finish = !0)), p.tmp.repulse_clicking ? (i = Math.pow(p.interactivity.modes.repulse.distance / 6, 3), n = p.interactivity.mouse.click_pos_x - a.x, s = p.interactivity.mouse.click_pos_y - a.y, c = -i / (r = n * n + s * s) * 1, r <= i && function() {
            var e, t = Math.atan2(s, n);
            a.vx = c * Math.cos(t), a.vy = c * Math.sin(t), "bounce" == p.particles.move.out_mode && (e = a.x + a.vx, t = a.y + a.vy, (e + a.radius > p.canvas.w || e - a.radius < 0) && (a.vx = -a.vx), (t + a.radius > p.canvas.h || t - a.radius < 0) && (a.vy = -a.vy))
...
var Stats = function() {
    ...
    var n, s = (performance || Date).now(),
        r = s,
        c = 0,
        o = e(new Stats.Panel("FPS", "#0ff", "#002")),
        l = e(new Stats.Panel("MS", "#0f0", "#020"));
    return self.performance && self.performance.memory && (n = e(new Stats.Panel("MB", "#f08", "#201"))), t(0), {
        REVISION: 16,
        dom: i,
        addPanel: e,
        showPanel: t,
        begin: function() {
            ...
            return l.update(t - s, 200), r + 1e3 < t && (o.update(1e3 * c / (t - r), 100), r = t, c = 0, n) && (e = performance.memory, n.update(e.usedJSHeapSize / 1048576, e.jsHeapSizeLimit / 1048576)), t
        },
        update: function() {
            s = this.end()
        },
...
Stats.Panel = function(a, i, n) {
    ...
    return h.font = "bold " + 9 * o + "px Helvetica,Arial,sans-serif", h.textBaseline = "top", h.fillStyle = n, h.fillRect(0, 0, l, e), h.fillStyle = i, h.fillText(a, v, p), h.fillRect(d, m, u, y), h.fillStyle = n, h.globalAlpha = .9, h.fillRect(d, m, u, y), {
        dom: b,
        update: function(e, t) {
            s = Math.min(s, e), r = Math.max(r, e), h.fillStyle = n, h.globalAlpha = 1, h.fillRect(0, 0, l, m), h.fillStyle = i, h.fillText(c(e) + " " + a + " (" + c(s) + "-" + c(r) + ")", v, p), h.drawImage(b, d + o, m, u - o, y, d, m, u - o, y), h.fillRect(d + u - o, m, o, y), h.fillStyle = n, h.globalAlpha = .9, h.fillRect(d + u - o, m, o, c((1 - e / t) * y))
        }
    }
